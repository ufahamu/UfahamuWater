...
function get_category_home()
{
	...
	//$catsql = "select * from $wpdb->terms c,$wpdb->term_taxonomy tt where tt.term_id=c.term_id and c.term_id in ($map_cat_ids) and tt.taxonomy in ('category','".CUSTOM_CATEGORY_TYPE1."','".CUSTOM_CATEGORY_TYPE2."') order by c.term_id,c.name";	
	...
	{
		$catsql = "select * from $wpdb->terms c,$wpdb->term_taxonomy tt  where tt.term_id=c.term_id and tt.taxonomy in ('category','".CUSTOM_CATEGORY_TYPE1."','".CUSTOM_CATEGORY_TYPE2."') and count > 0 ";
...
/* 				$sql = "select * from $wpdb->posts p join $wpdb->postmeta pm on pm.post_id=p.ID where p.post_status in ('publish') and p.post_type in ($my_post_type) and (pm.meta_key='post_city_id' and pm.meta_value = $multi_city_id) and p.ID in (select tr.object_id from $wpdb->term_relationships tr join $wpdb->term_taxonomy t on t.term_taxonomy_id=tr.term_taxonomy_id where t.term_id=\"$term_id\" )"; */
			...
			if($postinfo)
			{
				...
				foreach($postinfo as $postinfo_obj)
				{
					...
					$pimg = $pimgarr[0];
					$thumb = templ_thumbimage_filter($pimg,'&amp;w=90&amp;h=70&amp;zc=1&amp;q=80',1);
					if($lat && $lng)
					{
						$retstr ="{";
						$retstr .= "'name':'$title',";
						$retstr .= "'location': [$lat,$lng],";
						$retstr .= "'message':'<img src=\"$thumb\" width=\"90\" height=\"70\" style=\"float:left; margin:0 11px 22px 0;\" alt=\"\" />";
...
<div class="top_banner_section" >
        ...
        <?php for($c=0;$c<count($catname_arr);$c++){ ?>
         <label><input type="checkbox" value="<?php echo $catname_arr[$c][1];?>" checked="checked" id="<?php echo $catname_arr[$c][1];?>" name="<?php echo $catname_arr[$c][1];?>"><img height="14" width="8" alt="" src="<?php echo $catname_arr[$c][2];?>"> <?php echo $catname_arr[$c][0];?></label>
