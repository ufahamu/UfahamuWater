...
function get_wp_category_checklist($post_taxonomy,$pid)
{
        ...
        WHERE {$table_prefix}terms.term_id = {$table_prefix}term_taxonomy.term_id
                AND ({$table_prefix}term_taxonomy.taxonomy ='".CUSTOM_CATEGORY_TYPE2."' or {$table_prefix}term_taxonomy.taxonomy ='".CUSTOM_CATEGORY_TYPE1."')  ORDER BY {$table_prefix}terms.name");
		}else{
		$wpcategories = (array)$wpdb->get_results("
        SELECT * FROM {$table_prefix}terms, {$table_prefix}term_taxonomy
        WHERE {$table_prefix}terms.term_id = {$table_prefix}term_taxonomy.term_id
		...
		if($taxonomy !=""){
		 ...
		 ?>
			<li style="margin-left:15px;"><label><input type="checkbox" name="category[]" id="<?php echo $termid; ?>" value="<?php echo $termid; ?>" class="checkbox" <?php if($pid[0]){ if(in_array($termid,$pid)){ echo "checked=checked"; } }else{  }?> /></label>&nbsp;<?php echo $name; if($termprice != "") { echo " (".display_amount_with_currency($termprice).") ";}else{  echo " (".display_amount_with_currency('0').") "; } ?></li>
		<?php }	}	
}
	echo "</ul>"; } 
}
