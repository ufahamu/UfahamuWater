				...
				$category = explode("-",$cat);
					...
					if($_REQUEST['pid']){
						if($place_info['renew']) {	
							...
							$cat_cntcat = $wpdb->query("UPADTE " . $termtable . " set object_id = '".$post_id."',term_taxonomy_id = '".$place_info['category']."',term_order = '0')");	
						}
					} else {
						$cat_cntcat = $wpdb->query("INSERT INTO " . $termtable . "(object_id, term_taxonomy_id, term_order)VALUES ('".$post_id."','".$place_info['category']."','0') ");
					}
			...
			foreach($_SESSION["file_info"] as $arrVal)
			 {
				...
				$expName = array_slice(explode("-",$expName[0]),0,1);
