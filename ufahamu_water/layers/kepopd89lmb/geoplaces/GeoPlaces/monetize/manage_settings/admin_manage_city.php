...
<!--<p style="margin-top:25px; position:relative;"><?php echo _e('Current default city is: ','templatic'); ?><span id='cityprocess' style='display:none;'><?php _e('Saving changes...','templatic'); ?>&nbsp;<img style="position:absolute; top:-2px;" src="<?php echo get_template_directory_uri()."/images/process.gif"; ?>" alt='<?php _e('Saving changes...','templatic'); ?>' /></span><strong id="set_default_city"><?php if($cityinfo) {
	foreach($cityinfo as $cityinfoObj)	{ if($cityinfoObj->is_default =='1' ){?><?php echo $cityinfoObj->cityname;?> <?php } } } ?></strong></p>
    
<p style="padding-bottom:0;"><label><input type="checkbox" /><?php echo _e('Enable users to select a city when they land on your website','templatic'); ?></label></p>
<p style="margin-left:20px;"><small><?php echo _e('(This won&rsquo;t change the &lsquo;Default city&rsquo; selection above, but will switch the homepage map to user-selected city)','templatic'); ?></small></p>-->
