...
function zones_cmb($country_id,$zones_id = ''){
	global $zones_table;
	$zones_sql = mysql_query("select zones_id,zone_name from $zones_table where country_id = '".$country_id."'");
	$zones_display = '';
	$zones_select = "";
	while($zones_res = mysql_fetch_array($zones_sql)){
		if($zones_res['zones_id'] == $zones_id){
			$zones_select = "selected";
		} else {
			$zones_select = "";
		}
		$zones_display .= '<option value="'.$zones_res['zones_id'].'" '.$zones_select.'>'.$zones_res['zone_name'].'</option>';
	}
